...
ibuki.filterOn('parseXml:util:xmlParse').subscribe(
    d => {
        ...
        switch (carrierInfo.name) {
            case 'UPS':
                parseXML.getUpsData(d.data.response, packageStatus,carrierInfo);
                break;
            case ("FEX"):
            case ("FCC"):
                parseXML.getFedexData(d.data.response, packageStatus,carrierInfo);
                break;
            case ("GSO"):
                parseXML.getGSOData(d.data.response, packageStatus,carrierInfo);
                break;
            case ("TPS"):
                parseXML.getTPSData(d.data.response, packageStatus,carrierInfo);
                break;
        }
    }
);
...
parseXML.getGSOData = (response, packageStatus) => {
    packageStatus.responseStatus = response.ShipmentInfo[0].Delivery.TransitStatus
}
parseXML.getTPSData = (response, packageStatus) => {

}
parseXML.getUpsData = (response, packageStatus,carrierInfo) => {
    parser.parseString(response, function (err, result) {
        if (result.TrackResponse.Response[0].ResponseStatusDescription.toString() == "Success" ) {
            packageStatus.ResponseStatus =result['TrackResponse']['Shipment'][0]['Package'][0]['Activity'][0]['Status'][0]['StatusType'][0]['Code'][0];
            packageStatus.ResponseStatusCode =result['TrackResponse']['Shipment'][0]['Package'][0]['Activity'][0]['Status'][0]['StatusType'][0]['Description'][0];
            //config.buffer.next({ trackingNumber: carrierInfo.trackingNumber, name: carrierInfo.name });
            ibuki.emit('sql1-update:util>db1',{rn:carrierInfo.rn,trackingNumber: carrierInfo.trackingNumber, name: carrierInfo.name,packageStatus:packageStatus});
...
// parseXML.getUpsData = (response, packageStatus, activityArray) => {

//     parser.parseString(response, function (err, result) {
//         if (result.TrackResponse.Response[0].ResponseStatusDescription.toString() == "Success") {
//             let steps = {};
//             let activity = result.TrackResponse.Shipment[0].Package[0].Activity;
//             console.log(activity.length);
//             activity.forEach(item => {
//                 console.log(item['Date'][0],item['Time'][0],item['Status'][0].StatusType[0].Description.toString()); 
//                 // Object.keys(item).forEach(key => {
//                 //     console.log(key); // returns the keys in an object
//                 //     (key == 'Status') && (() => {                       
//                 //         steps.Code =item[key][0].StatusType[0].Description.toString();
//                 //     })
...
//         }
//         //console.log(result.TrackResponse.Shipment[0].Package[0].Activity[0].Status[0].StatusType[0].Description[0].toString());
//     });
